...
<template x-teleport="body">
    <div
        x-data="{ sizes: ['sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl', '5xl'] }"
        x-dialog
        x-model="dialogOpen"
    ...
    >
        <div
            ...
            @class([
                ...
                'backdrop-blur cursor-not-allowed' => $blur,
            ])
        ></div>
        ...
        <div
            ...
            @else
                x-show="dialogOpen"
            @endif
            class="relative w-full origin-top rounded-lg bg-white transition-all"
            :class="{
                'max-w-sm': sizes[0] === '{{ $size ?? '' }}',
                'max-w-md': sizes[1] === '{{ $size ?? '' }}',
                'max-w-lg': sizes[2] === '{{ $size ?? '' }}',
                'max-w-xl': sizes[3] === '{{ $size ?? '' }}',
                'max-w-2xl': sizes[4] === '{{ $size ?? '' }}',
                'max-w-3xl': sizes[5] === '{{ $size ?? '' }}',
                'max-w-4xl': sizes[6] === '{{ $size ?? '' }}',
                'max-w-5xl': sizes[7] === '{{ $size ?? '' }}',
            }"
        ...
        >
            ...
            @else
                <div class="flex justify-between items-center rounded-t-lg bg-gradient-to-r from-blue-700 to-blue-500 text-white px-4 py-3 sm:px-5">
                    ...
                    <button
                        type="button"
                        x-on:click="$dialog.close(); $dispatch('dialog:close')"
                ...
                <div class="px-4 py-4 sm:px-5">
                    <div {{ $attributes->merge(['class' => 'max-h-[60vh] lg:max-h-[70vh] xl:max-h-[80vh] overflow-y-scroll is-scrollbar-hidden']) }}>
                        {{ $slot }}
                    </div>
                    @if ($footer ?? false)
                        <div {{ $footer->attributes->merge(['class' => 'text-right']) }}>
                            {{ $footer }}
                        </div>
                    @endif
                </div>
