...
<div x-data="otherMerchants" class="col-span-2 mt-20 w-full p-4 sm:p-5" :class="{
        '!mt-4' : method === 'helping-hand-link',
        'card' : method !== 'helping-hand-link'
    }" x-on:update-card-number.window="card_number = ''">
    <x-loader wire:loading wire:target="makePayment" />
    ...
    <div x-show="method === @js(MerchantType::CC->value)">
        <div
            ...
            <div
                class="size-full rounded-lg bg-[conic-gradient(at_bottom_right,_var(--tw-gradient-stops))] from-teal-600 via-blue-200 to-neutral-100">
            </div>
            <div class="absolute top-0 flex size-full flex-col justify-between p-4 sm:p-5">
                <div class="flex justify-between">
                    <div>
                        ...
                        <span x-text="$wire.form.card_holder_name"
                            class="font-bold capitalize tracking-wide text-xl text-primary"></span>
                    </div>
    ...
    <div x-show="method === @js(MerchantType::CC->value)" class="flex items-center justify-between py-4">
    ...
    <template x-if="method === @js(MerchantType::CC->value)">
        <form id="make-payment-form" wire:submit="makePayment" autocomplete="off">
            <div class="space-y-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div>
                        <label class="block">
                            ...
                            <span class="relative mt-1.5 flex">
                                <input type="tel" x-model="card_number"
                                    ...
                                    placeholder="{{ __('Card Number') }}" autocomplete="off" required>
                            </span>
                            <div x-data="{
                                ...
                                <x-consumer.dialog>
                                    <x-consumer.dialog.open>
                                        <span
                                            class="text-blue-600 text-lg font-semibold hover:underline cursor-pointer">
                                            {{ __('View Saved Cards') }}
                                        </span>
                                    </x-consumer.dialog.open>
                                    ...
                                    <x-consumer.dialog.panel :heading="__('Saved Cards')" class="h-[400px]">
                                        <div class="space-y-4 px-4 py-2 overflow-y-auto">
                                            ...
                                            <div
                                                class="p-4 border rounded-lg shadow-sm flex justify-between items-center bg-white hover:bg-gray-50 transition-all">
                                                <div class="flex-1">
                                                    <p class="font-semibold text-lg">{{ $card->card_holder_name }}</p>
                                                    <p class="text-sm text-gray-500">**** **** **** {{ $card->last4digit
                                                        }}</p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="block">
                            ...
                            <span class="relative mt-1.5 flex">
                                <input type="text" x-on:input="expiry" x-on:blur="generateExpiry" x-mask="99/9999"
                                    maxlength="7" pattern="(0[1-9]|1[0-2])\/\d{4}"
                                    class="form-input peer w-full rounded-lg border border-slate-400 bg-transparent px-3 py-2 placeholder:text-slate-400 hover:border-slate-400 focus:border-accent"
                                    placeholder="MM/YY" inputmode="numeric" autocomplete="off" required
                        ...
                        <div x-data="{ showCvv: false }">
                            <label class="block">
                                ...
                                <div class="relative flex mt-1.5">
                                    <input x-bind:type="showCvv ? 'text' : 'password'" wire:model="form.cvv"
                                        ...
                                        placeholder="232" maxlength="4" pattern="\d*" inputmode="numeric"
                                        autocomplete="off" x-on:input="$el.value = $el.value.replace(/\D/g, '')"
                                        required />
                                    <div
                                        class="absolute right-0 flex h-full w-10 items-center justify-center text-slate-400">
                                        <x-heroicon-o-eye-slash x-on:click="showCvv = !showCvv" x-show="!showCvv"
                                            class="w-5.5" />
                                        <x-heroicon-o-eye x-on:click="showCvv = !showCvv" x-show="showCvv"
                                            class="w-5.5" />
                                    </div>
                                </div>
                                @error('form.cvv')
                ...
                <label class="block">
                    ...
                    <span class="relative mt-1.5 flex">
                        <input type="text" wire:model="form.card_holder_name"
                            class="form-input peer w-full rounded-lg border border-slate-400 bg-transparent px-3 py-2 placeholder:text-slate-400 hover:border-slate-400 focus:border-accent"
                            placeholder="{{ __('Name on Card') }}" autocomplete="off" required>
                    </span>
                    ...
                    <div>
                        <label class="flex flex-col md:flex-row mt-3">
                            <div class="inline-flex items-start gap-2">
                                <div class="shrink-0">
                                    <input wire:model="form.is_terms_accepted" type="checkbox"
                                ...
                                <div class="xl:mt-0.5">
                                    ...
                                    <x-consumer.dialog class="inline-block">
                                        <span x-on:click="$event.preventDefault(); dialogOpen = true"
                                            class="underline underline-offset-2 cursor-pointer text-nowrap">
                                            {{ __('Terms & Conditions') }}
                                        </span>

                                        <x-consumer.dialog.panel :heading="__('Terms and Condition')" size="2xl"
                                            class="h-96">
                                            @if ($termsAndCondition)
                                            <div class="ql-editor">
                                                {!! $termsAndCondition !!}
                ...
                <div class="flex justify-center space-x-2">
                    <button type="submit"
                        ...
                        wire:loading.class="opacity-50" wire:loading.attr="disabled" wire:target="makePayment"
            ...
            <div class="absolute top-0 flex size-full flex-col justify-between p-4 sm:p-5">
                <div class="flex justify-between">
                    <div>
                        ...
                        <span x-text="$wire.form.account_number"
                            class="font-bold tracking-wide text-xl text-primary"></span>
                    </div>
                ...
                <div class="flex justify-between">
                    <div>
                        ...
                        <span x-text="$wire.form.routing_number"
                            class="font-bold tracking-wide text-xl text-primary"></span>
                    </div>
                </div>
            </div>
    ...
    <div x-show="method === @js(MerchantType::ACH->value)" class="flex items-center justify-between py-4">
        <p class="text-lg sm:text-xl font-semibold text-primary">
            {{ __('Account Details') }}
        </p>
    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    <div class="lg:col-span-5">
                        <label class="block">
                            ...
                            <span class="relative mt-1.5 flex">
                                <input type="text" pattern="[0-9.]+" wire:model="form.account_number"
                                    class="form-input peer w-full rounded-lg border border-slate-400 bg-transparent px-3 py-2 placeholder:text-slate-400 hover:border-slate-400 focus:border-accent"
                                    placeholder="34122345982" minlength="4" maxlength="17" autocomplete="off" required>
                            </span>
                    ...
                    <div class="lg:col-span-4">
                        <label class="block">
                            ...
                            <span class="relative mt-1.5 flex">
                                <input type="text" pattern="[0-9.]+" wire:model="form.routing_number"
                                    class="form-input peer w-full rounded-lg border border-slate-400 bg-transparent px-3 py-2 placeholder:text-slate-400 hover:border-slate-400 focus:border-accent"
                                    placeholder="022345982" minlength="9" maxlength="9" autocomplete="off" required>
                            </span>
