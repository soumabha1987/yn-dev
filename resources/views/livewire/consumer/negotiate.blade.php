...
<div>
    <main class="w-full pb-8">
        <div class="flex flex-col lg:flex-row items-start pb-6 gap-6 sm:gap-4">
            <div class="flex card pt-6 pb-6 items-start w-full lg:w-1/3">
                <div class="flex flex-row lg:flex-col items-center lg:items-start w-full justify-between">
                    <div class="px-6">
                        <x-consumer.creditor-details :$creditorDetails>
                            <h2 class="uppercase text-primary text-lg lg:text-xl line-clamp-1 font-semibold cursor-pointer hover:underline hover:underline-offset-2 decoration-primary">
            ...
            <div
                x-on:dialog:close.window="$wire.first_pay_date = ''"
                class="flex flex-col space-y-4 w-full lg:w-2/3"
            >
                <div 
                    class="flex cursor-pointer card rounded-lg px-4 sm:px-6 py-6 sm:py-8 items-center min-h-[140px] sm:min-h-[180px] hover:outline outline-primary"
                    wire:click="createSettlementOfferWithToday"
                >
                    <div class="flex flex-wrap w-full">
                        <div class="w-full md:w-2/3">
                            <h3 class="text-base md:text-lg font-semibold text-black">
                                {{ __('One time payment of :payOffDiscountBalance', ['payOffDiscountBalance' => Number::currency((float) $payOffDiscount)]) }}
                            </h3>
                            ...
                            <div class="flex justify-between w-full">
                                ...
                                <div class="sm:hidden flex flex-col content-center justify-end">
                                    ...
                                    @if ($payOffDiscountedAmount > 0)
                                        <div class="bg-error px-3 py-1 text-white text-xs+ font-bold text-center rounded-full mt-1">
                        ...
                        <div class="w-full md:w-1/3">
                            <div class="hidden sm:flex flex-wrap content-center justify-end h-full">
                                <div class="min-w-[86px]">
                                    ...
                                    @if ($payOffDiscountedAmount > 0)
                                        <div class="bg-error px-2.5 sm:px-3 py-1.5 sm:py-2 text-white text-xs sm:text-sm text-center sm:text-left rounded-full mt-2">
                                            {{ __(':off  Off', ['off' => Number::percentage((float) $payOffDiscountPercentage)]) }}
                                        </div>
                                    @endif
                                </div>
                ...
                <x-consumer.dialog>
                    <x-consumer.dialog.open>
                        <div class="flex cursor-pointer card rounded-lg px-4 sm:px-6 py-6 sm:py-8 items-center min-h-[140px] sm:min-h-[180px] hover:outline outline-primary">
                            <div class="flex flex-wrap w-full">
                                <div class="w-full md:w-2/3">
                                    <h3 class="text-base md:text-lg font-semibold text-black">
                                    ...
                                    <p class="text-xs md:text-sm text-black">
                                        {{ __('I am excited to pay this in :months payments', ['months' => $installmentDetails['installments']]) }}
                                    </p>
                                        ...
                                        @if ($installmentDetails['discounted_amount'] > 0)
                                            <div class="block">
                                                <p class="text-primary font-bold text-xl mt-3">
                                                    {{ __('Save :amount', ['amount' => Number::currency((float) $installmentDetails['discounted_amount'])]) }}
                                                </p>
                                            </div>
                                        @endif
                    ...
                    <x-consumer.dialog.panel
                        :heading="__('Choose First Payment Date')"
                        class="h-[400px]"
                    >
                        <form
                            x-ref="installmentOfferFrom"
                            wire:submit="createInstallmentOffer"
                            autocomplete="off"
                        >
                            <div class="mx-5">
                                <x-consumer.offers.first-pay-date
                                    :$maxFirstPayDate
                                    modelable="first_pay_date"
                                />
                            </div>
                            <x-slot
                                name="footer"
                ...
                <a
                    wire:navigate
                    href="{{ route('consumer.custom-offer', ['consumer' => $consumer]) }}"
                >
                    <div class="flex cursor-pointer card rounded-lg px-4 sm:px-6 py-6 sm:py-8 items-center min-h-[140px] sm:min-h-[180px] hover:outline outline-primary">
                        <div class="flex flex-wrap w-full">
                            <div class="w-full md:w-2/3">
