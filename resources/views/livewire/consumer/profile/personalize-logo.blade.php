<x-consumer.profile-layout route-name="personalize-your-experience">
    <div class="card">
        <div class="flex flex-col items-center space-y-4 border-b border-slate-200 p-4 sm:flex-row sm:justify-between sm:space-y-0 sm:px-5">
            ...
            <div class="hidden sm:flex justify-center space-x-2">
                <button
                    ...
                    class="btn min-w-[7rem] rounded-full border border-red-300 font-medium text-red-700 hover:bg-red-100 focus:bg-red-150 active:bg-red-100/80"
                >
                    {{ __('Reset') }}
                </button>
                <x-form.button
                    type="button"
                    variant="primary"
                    class="min-w-[7rem] rounded-full font-medium"
                    wire:click="createOrUpdate"
                    wire:target="createOrUpdate, form.image"
        ...
        <div class="p-4 sm:p-5">
            <div
                ...
                x-on:livewire-upload-start="$wire.resetFormImageValidation()"
            >
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    @if ($form->image && in_array($form->image->getMimeType(), ['image/jpeg', 'image/png', 'image/jpg']))
                        <img
                    ...
                    <div class="mt-1">
                        ...
                        @enderror
                        @if($form->image && !$errors->has('form.image') && ! in_array($form->image->getMimeType(), ['image/jpeg', 'image/png', 'image/jpg']))
                            <span class="text-error">
            ...
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <div>
                    <label class="block">
                        <span class="font-semibold text-black">{{ __('Select Primary Color') }}</span>
                        <div class="relative mt-1.5 flex">
                            <input
                                type="color"
                                wire:model="form.primary_color"
                                @class([
                                    'form-input peer w-full rounded border bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary h-8 appearance-none',
                                    'border-error' => $errors->has('form.primary_color'),
                                    'border-slate-300' => $errors->missing('form.primary_color'),
                                ])
                ...
                <div>
                    <label class="block">
                        <span class="font-semibold text-black">{{ __('Select Secondary Color') }}</span>
                        <span class="relative mt-1.5 flex">
                            <input
                                type="color"
                                wire:model="form.secondary_color"
                                @class([
                                    'form-input peer w-full rounded border bg-transparent px-3 py-2 placeholder:text-slate-400/70 h-8 hover:border-slate-400 focus:border-primary',
                                    'border-error' => $errors->has('form.secondary_color'),
                                    'border-slate-300' => $errors->missing('form.secondary_color'),
                                ])
                                placeholder="{{ __('Select secondary color') }}"
                            >
                        </span>
                        @error('form.secondary_color')
                            <div class="mt-1">
                                <span class="text-error">
                                    {{ $message }}
                                </span>
                ...
                <div class="mt-8 flex justify-center text-4xl font-bold">
                    <svg viewBox="-1135.6452 -3.6259 3009.5033 383.13" xmlns="http://www.w3.org/2000/svg" width="400px"><script xmlns=""/>
                        ...
                        <g transform="matrix(1.0358690023422241, 0, 0, 1.0989489555358887, -1694.444580078125, -946.2471923828125)" :style="'fill:'+ $wire.form.secondary_color +'!important;'">
                            ...
                            <text style="font-family: Arial, sans-serif; font-size: 22.4px; white-space: pre;" x="3438.51" y="1007.84"> </text>
                        </g>
                        <g transform="matrix(0.9999999999999999, 0, 0, 0.9999999999999999, -1578.105224609375, -858.02587890625)" :style="'fill:'+ $wire.form.secondary_color +'!important;'">
            ...
            {{-- mobile view --}}
            <div class="flex sm:hidden justify-center space-x-2 mt-4">
                <button
                    wire:click="resetForm"
                    class="btn min-w-[7rem] rounded-full border border-red-300 font-medium text-red-700 hover:bg-red-100 focus:bg-red-150 active:bg-red-100/80"
                >
