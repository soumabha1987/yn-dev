...
<div class="grid grid-cols-1 gap-4 sm:gap-5 lg:gap-6 w-full">
    ...
    >
        ...
        <div class="card py-4 px-4 sm:px-5">
            ...
            <div x-data="{ templateType: @js($form->type) }">
                <template x-if="templateType === '{{ AutomatedTemplateType::EMAIL->value }}'">
                    <div
                        x-data="{ emailSubject: null }"
                        x-on:clear-subject.window="emailSubject = null"
                        class="mt-5"
                    >
                        <div class="mt-5">
                            ...
                            <p>
                                @foreach(TemplateCustomField::values() as $field)
                                    <button
                                        type="button"
                        ...
                        <div class="mt-5">
                            <div class="mb-5">
                                <div class="my-1.5 flex items-center justify-between">
                                    <h2 class="font-medium capitalize tracking-wide text-black lg:text-sm+">
                                        {{ __('Select custom data for email content') }}
                                    </h2>
                                </div>
                                <p>
                                    @foreach(TemplateCustomField::values() as $field)
                            ...
                            <div class="my-1.5 flex items-center justify-between">
                                <h2 class="font-semibold tracking-wide text-black lg:text-md">
                                    {{ __('Email Template Content') }}
                                    <sup class="text-error text-base">*</sup>
                                </h2>
                            </div>
                            <x-form.quill-editor
                                class="h-48"
                ...
                <template x-if="templateType === '{{ AutomatedTemplateType::SMS->value }}'">
                    <div class="mt-5">
                        <div class="mb-5">
                            <div class="my-3 flex items-center justify-between">
                                <h2 class="font-medium capitalize tracking-wide text-slate-700 line-clamp-1 lg:text-base">
                                    {{ __('Select custom data for sms content') }}
                                </h2>
                            </div>
                            <p>
                                @foreach(TemplateCustomField::values() as $field)
