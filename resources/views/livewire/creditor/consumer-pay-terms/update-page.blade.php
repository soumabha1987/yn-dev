<div>
    <x-dialog>
        <x-dialog.open>
            @if ($isMenuItem)
                <x-menu.item>
                    <x-lucide-circle-percent class="size-4.5 sm:size-5"/>
                    {{ __('Set Pay Terms') }}
                </x-menu.item>
            @else
                <x-form.button
                    ...
                    variant="primary"
                    @class([
                        'border focus:border-primary-focus w-full',
                        'text-xs sm:text-sm+ py-1.5 px-2 sm:px-3 whitespace-nowrap' => ! $isConsumerPayTerms
                    ])
                >
                    <x-lucide-circle-percent class="size-4.5 sm:size-5 mr-2"/>
                    {{ $isConsumerPayTerms ? __('Special Pay Term') : __('Pay Terms Offers') }}
                </x-form.button>
            @endif
        </x-dialog.open>

        <div>
            <x-dialog.panel size="5xl">
                <x-slot name="heading">{{ __('Update Consumer Pay Terms') }}</x-slot>
                <form
                    wire:submit="update"
                    autocomplete="off"
                    x-ref="updateConsumerPayTermForm"
                >
                    <div
                    ...
                    >
                        <div
                            class="inline-block w-fit font-semibold text-success cursor-pointer"
                        ...
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
                            <div class="rounded-lg border bg-white-150 p-2">
                                <div class="flex flex-col justify-between h-full">
                                    <span class="font-semibold tracking-wide text-black lg:text-md">
                                        {{ __('Pay In Full Balance Discount %') }}<span class="text-error text-base">*</span>
                                    </span>
                                    <p
                                    ...
                                    <div class="mt-auto">
                                        <x-form.input-field
                                            wire:model="{{ $isConsumerPayTerms ? 'form.pif_discount_percent' : 'form.pif_balance_discount_percent' }}"
                                            type="number"
                                            step="1"
                                            name="{{ $isConsumerPayTerms ? 'form.pif_discount_percent' : 'form.pif_balance_discount_percent' }}"
                            ...
                            <div class="rounded-lg border bg-white-150 p-2">
                                <div class="flex flex-col justify-between h-full">
                                    ...
                                    <div class="mt-auto">
                                        <x-form.input-field
                                            wire:model="{{ $isConsumerPayTerms ? 'form.pay_setup_discount_percent' : 'form.ppa_balance_discount_percent' }}"
                                            type="number"
                                            step="1"
                                            name="{{ $isConsumerPayTerms ? 'form.pay_setup_discount_percent' : 'form.ppa_balance_discount_percent' }}"
                            ...
                            <div class="rounded-lg border bg-white-150 p-2">
                                <div class="flex flex-col justify-between h-full">
                                    <span class="font-semibold tracking-wide text-black lg:text-md">
                                        {{ __('Maximum Days to First Payment') }}<span class="text-error">*</span>
                                    </span>
                                    <p
                                        x-show="isOpenDescription"
                                        x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
                            <div class="rounded-lg border bg-white-150 p-2">
                                <div class="flex flex-col justify-between h-full">
                                    <span class="font-semibold tracking-wide text-black lg:text-md">
                                        {{ __('Minimum Pay In Full Discount %') }}<span class="text-error">*</span>
                                    </span>
                                    <p
                                        x-show="isOpenDescription"
                                        x-transition:enter="transition ease-out duration-300"
                                    ...
                                    <div class="mt-auto">
                                        <x-form.input-field
                                            ...
                                            :placeholder="__('% of minimum settlement')"
                                            class="w-full"
                                        />
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col justify-between h-full">
                                    ...
                                    <div class="mt-auto">
                                        <x-form.input-field
                                            ...
                                            :placeholder="__('Max first payment date')"
                                            class="w-full"
                                        />
                                    </div>
                                </div>
                            ...
                            <div class="rounded-lg border bg-white-150 p-2">
                                <div class="flex flex-col justify-between h-full">
                                    <span class="font-semibold tracking-wide text-black lg:text-md">
                                        {{ __('Minimum Monthly Payment % of Balance') }}<span class="text-error">*</span>
                                    </span>
                                    <p
                                        x-show="isOpenDescription"
                                        x-transition:enter="transition ease-out duration-300"
                        ...
                        <x-slot name="footer" class="flex justify-end space-x-2 mt-4">
                            <x-dialog.close>
                            ...
                            <x-form.button
                                type="submit"
                                variant="primary"
                                x-on:click="$refs.updateConsumerPayTermForm.requestSubmit()"
                                class="border focus:border-primary-focus font-medium w-24"
                            >
                                {{ __('Update') }}
                            </x-form.button>
                        </x-slot>
