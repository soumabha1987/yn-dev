            ...
            <x-table>
                <x-slot name="tableHead">
                    <x-table.tr>
                        ...
                        <x-table.th column="amount" :$sortCol :$sortAsc>{{ __('Transaction Amount') }}</x-table.th>
                    </x-table.tr>
                </x-slot>
                <x-slot name="tableBody">
                    @forelse ($scheduleTransactions as $scheduleTransaction)
                        <x-table.tr>
                            <x-table.td>{{ $scheduleTransaction->last_attempted_at->formatWithTimezone(format: 'M d, Y h:i A') }}</x-table.td>
                            <x-table.td>
                                <a
                                    wire:navigate
                            ...
                            <x-table.td>{{ $scheduleTransaction->transaction->transaction_id ?? 'N/A' }}</x-table.td>
                            <x-table.td>{{ Number::currency((float) ($scheduleTransaction->amount ?? 0)) }}</x-table.td>
                        </x-table.tr>
                    @empty
